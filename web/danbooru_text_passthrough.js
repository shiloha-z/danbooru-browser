  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const buildUi = () => {
        ...
        if (!textWidget) return false;  // widget 在 onNodeCreated 后才创建,延时重试

        const wrap = document.createElement("div");
        wrap.style.cssText = "display:flex;flex-direction:column;gap:4px;padding:6px;min-width:220px;min-height:140px;height:100%;box-sizing:border-box;overflow-y:auto";
      ...
      if (!buildUi()) setTimeout(buildUi, 300);  // widget 未就绪时重试
    };
  },
});
